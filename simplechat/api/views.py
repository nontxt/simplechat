...
def is_not_current_user(request, **kwargs):
    """Return User if exists and is not current"""
...
class MessageListCreateAPIView(ListCreateAPIView):
    ...
    def get_thread(self, interlocutor=None):
        """ Return a thread for current user and interlocutor"""
        user_threads = self.request.user.threads.all()
        thread_id_list = user_threads.values_list('id')

        self.thread = get_object_or_404(Thread, Q(id__in=thread_id_list) & Q(participants=interlocutor))
        return self.thread

    def get_queryset(self):
        """ Return a list of messages for current user and interlocutor"""
...
class UnreadMessageListCreateAPIView(ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(queryset, many=True)
        return Response({'count': queryset.count(), 'messages': serializer.data})
